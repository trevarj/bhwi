...
impl ColdcardError {
    pub fn unexpected_response_message(
        ...
        ColdcardError::UnexpectedResponseMessage { got, expected: expected.to_vec() }
    }
}
    ...
    fn exchange(&mut self, data: Vec<u8>) -> Result<Option<Self::Transmit>, Self::Error> {
        match &self.state {
            ...
            State::Running(ColdcardCommand::SignMessage { .. }) => {
                ...
                if let ColdcardResponse::Ok = res {
                    return Ok(Some(
...
impl From<FromUtf8Error> for ColdcardError {
    fn from(error: FromUtf8Error) -> Self { ColdcardError::Serialization(error.to_string()) }
}
