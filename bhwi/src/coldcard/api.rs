pub mod request {
    ...
    pub fn get_signed_message() -> Vec<u8> { b"smok".to_vec() }
}
...
pub mod response {
    ...
    pub fn master_fingerprint(res: &[u8]) -> Result<ColdcardResponse, ColdcardError> {
        Ok(ColdcardResponse::MasterFingerprint(xpub(res)?.fingerprint()))
    }
    ...
    pub fn mypub(res: &[u8]) -> Result<ColdcardResponse, ColdcardError> {
        ...
        let xpub_fingerprint = data
            .get(0..4)
            .ok_or(ColdcardError::Serialization("xfp wants 4 bytes".to_string()))?
            .try_into()
            .expect("infallible");
        ...
        Ok(ColdcardResponse::MyPub { encryption_key, xpub_fingerprint, xpub })
    }
    ...
    pub fn sign_message(res: &[u8]) -> Result<ColdcardResponse, ColdcardError> {
        match ResponseHandler::parse_response(res)? {
            ...
            (msg, _) => Err(ColdcardError::unexpected_response_message(
                msg,
                &[ResponseMessage::Okay, ResponseMessage::Smrx],
            )),
        }
    }
    ...
    fn split(bytes: &[u8], mid: usize) -> Result<(&[u8], &[u8]), ColdcardError> {
        match bytes.len().cmp(&mid) {
            std::cmp::Ordering::Less =>
                Err(ColdcardError::Serialization("unexpected slice length".to_string())),
    ...
    fn decode_u32(bytes: Option<&[u8]>) -> Result<u32, ColdcardError> {
        match bytes {
            Some(bytes) if bytes.len() == 4 => Ok(u32::from_le_bytes(
                bytes.try_into().map_err(|_| ColdcardError::Serialization("u32".to_string()))?,
            )),
